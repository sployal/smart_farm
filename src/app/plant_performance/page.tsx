...
const healthMeta: Record<HealthStatus, { label: string; color: string; glow: string; bg: string }> = {
  thriving: { label: 'Thriving', color: '#10b981', glow: '#10b98155', bg: 'from-emerald-950 to-slate-950' },
  good:     { label: 'Good',     color: '#06b6d4', glow: '#06b6d455', bg: 'from-cyan-950 to-slate-950'    },
  stressed: { label: 'Stressed', color: '#f59e0b', glow: '#f59e0b55', bg: 'from-amber-950 to-slate-950'   },
  critical: { label: 'Critical', color: '#ef4444', glow: '#ef444455', bg: 'from-red-950 to-slate-950'     },
};
...
function PlantAnimation({ status, moisture, health }: { status: HealthStatus; moisture: number; health: number }) {
  ...
  return (
    <div className="relative flex items-end justify-center w-full h-full">
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48 h-48 rounded-full blur-3xl opacity-30 transition-all duration-1000"
        style={{ background: glow }} />
      <svg viewBox="0 0 200 280" className="w-full max-w-[200px] drop-shadow-2xl"
        ...
        <defs>
          ...
          <linearGradient id="soilGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#713f12" /><stop offset="100%" stopColor="#92400e" />
          </linearGradient>
          <linearGradient id="waterGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#38bdf8" stopOpacity="0.6" /><stop offset="100%" stopColor="#0369a1" stopOpacity="0.9" />
          </linearGradient>
          <linearGradient id="stemGrad" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="#166534" /><stop offset="100%" stopColor="#15803d" />
          </linearGradient>
          <clipPath id="potClip"><path d="M 62 205 L 68 245 Q 100 252 132 245 L 138 205 Z" /></clipPath>
        </defs>
        ...
        <rect x="68" y={245 - (waterLevel / 100) * 32} width="64" height={(waterLevel / 100) * 32}
          fill="url(#waterGrad)" clipPath="url(#potClip)" className="transition-all duration-1000" />
        {moisture > 50 && (
        ...
        <path d={droopy ? "M 100 200 C 100 180 98 160 97 140 C 96 120 98 100 97 85" : "M 100 200 C 100 175 101 155 100 135 C 99 115 100 95 100 78"}
          stroke="url(#stemGrad)" strokeWidth="5" fill="none" strokeLinecap="round" className="transition-all duration-1000" />
        ...
        <g transform={`translate(65, 115) rotate(${droop}, 0, 0)`}>
          <ellipse cx="-14" cy="-8" rx="22" ry="12" fill={color} opacity={wilted ? 0.4 : 0.9} transform="rotate(-30)" className="transition-all duration-1000" />
          <line x1="-14" y1="-8" x2="-32" y2="-16" stroke="#166534" strokeWidth="1" opacity={wilted ? 0.3 : 0.6} />
        </g>
        ...
        <g transform={`translate(100, 78) rotate(${droop * 0.5}, 0, 0)`}>
          <ellipse cx="0" cy="-18" rx="16" ry="22" fill={color} opacity={wilted ? 0.35 : 0.95} className="transition-all duration-1000" />
          <ellipse cx="-16" cy="-22" rx="12" ry="16" fill={color} opacity={wilted ? 0.3 : 0.85} transform="rotate(-20)" className="transition-all duration-1000" />
          <ellipse cx="16" cy="-22" rx="12" ry="16" fill={color} opacity={wilted ? 0.3 : 0.85} transform="rotate(20)" className="transition-all duration-1000" />
          <line x1="0" y1="-4" x2="0" y2="-36" stroke="#166534" strokeWidth="1.5" opacity={wilted ? 0.3 : 0.7} />
        </g>
        {vibrant && [
      ...
      <div className="relative z-10 p-4 md:p-6 max-w-[1400px] mx-auto space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="card card-hover rounded-2xl overflow-hidden transition-all duration-300">
            ...
            <div className="relative h-72 flex items-end px-6 pt-4 pb-0 float">
          ...
          <div className="card card-hover rounded-2xl p-5 transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3 text-xs text-slate-500">
                ...
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-violet-400 inline-block" /> Health</span>
              </div>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={timeline}>
                  <defs>
                    <linearGradient id="gradH" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#10b981" stopOpacity={0.25} /><stop offset="95%" stopColor="#10b981" stopOpacity={0} />
                    </linearGradient>
